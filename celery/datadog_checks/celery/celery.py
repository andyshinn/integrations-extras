...
class CeleryCheck(AgentCheck):
    ...
    def check_connection(self, instance, tags):
        ...
        try:
            ...
            self.service_check(self.SERVICE_CHECK_NAME, AgentCheck.CRITICAL, tags=tags, message="Request timeout: {0}, {1}".format(url, e))
            raise
        except requests.exceptions.HTTPError as e:
            ...
            self.service_check(self.SERVICE_CHECK_NAME, AgentCheck.OK, tags=tags, message='Connection to %s was successful' % url)

    def _split_worker_name(self, instance, worker_name):
        ...
        """
        if instance.get('split_worker_name', self.SPLIT_NAME):
    ...
    def get_worker_data(self, instance, tags):
        ...
        for worker_name, worker_data in data.items():
            worker_name = self._split_worker_name(instance, worker_name)
    ...
    def get_task_data(self, instance, tags, workers):
        for worker in workers:
            worker_name = self._split_worker_name(instance, worker)
